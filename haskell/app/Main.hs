...
dayTwo input = horizontalPos * depth
  where
    (horizontalPos, depth) =
      foldl (\coord command -> parseCommand command coord) (0, 0) $
      map words input

parseCommand :: (Num a, Num b, Read a, Read b) => [String] -> (a, b) -> (a, b)
parseCommand xs coord =
  case xs of
...
two :: IO ()
two = do
  dayTwoInput <- readFile "input/two.txt"
  print $ dayTwo $ formatInput id dayTwoInput
...
interleave [] = []
interleave ([]:_) = []
interleave xs = map head xs : interleave (map tail xs)
